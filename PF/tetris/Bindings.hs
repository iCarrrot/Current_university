...
keyboardMouse ifFinish score pause blockNumber nextRan angle position table size key Down _ _ =  
  case key of
    (SpecialKey KeyLeft ) -> do
                            ...
                            let (left,right)=blockSize blockNumber' angle' in 
                              let size2 = 2*size' in
                              let (left',right')=blockSize blockNumber' (moduloGLfloat (angle'+1) 4) in
                                position $~! \(x,y) -> if checkIfEmptyX (block (getBlockId blockNumber') position' (0.05::GLfloat) angle') table' (-1::GLfloat) size2 >(0.1)
                                  then (x,y)
                                  else
    ...
    (SpecialKey KeyUp   ) -> do 
                            ...
                            size' <- get size
                            table' <- get table
                            if checkIfEmptyX (block (getBlockId blockNumber') position' (0.05::GLfloat) (moduloGLfloat (angle'+1) 4) ) table' (0::GLfloat) (2*size') <(0.5)
                              then do
    ...
    (SpecialKey KeyDown ) -> do
                            ...
                            table' <- get table
                            let size2 = 2*size' in
                              position $~! \(x,y) ->if (checkIfEmptyY (block (getBlockId blockNumber') position' (0.05::GLfloat) angle''  )  table' (2*size') )==1 
                                then (x,y) 
                                else
                                  if y>(-0.95+size2) 
                                    then(x,y-size2) 
